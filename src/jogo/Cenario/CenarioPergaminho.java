package jogo.Cenario;

import javax.swing.*;
import jogo.Som;
import jplay.*;

public class CenarioPergaminho {

    private final Window janela;
    private final Scene cena;
    private final Keyboard teclado;
    static boolean creditos;

    //Construtor
    public CenarioPergaminho(Window window) {

        janela = window;
        cena = new Scene();
        cena.loadFromFile(URL.scenario("CenarioPergaminho.scn"));
        teclado = janela.getKeyboard();
        creditos = false;

        run();

    }

    private void run() {
        while (true) {

            cena.draw();
            janela.update();
            lerTeclado();

            if (creditos == true) {
                Som.play("menu.mid");
                break;
            }
        }
    }

    public void lerTeclado() {
        if (teclado.keyDown(Keyboard.ESCAPE_KEY)) {
            if (JOptionPane.showConfirmDialog(null, "Você deseja mesmo sair?", "Sair", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, new ImageIcon("src/recursos/sprites/jogador-img.png")) == JOptionPane.YES_OPTION) {
                System.exit(0);
            }
        }

        if (teclado.keyDown(Keyboard.ENTER_KEY)) {
            Som.play("fundo.mid");
            new CenarioFora(janela, 200, 500, 3);

        }
    }
}
